...
Citizen.CreateThread(function()
    while true do
		...
		local play, distance = ESX.Game.GetClosestPlayer() -------- Se hace un chequeo del jugador mas cercano
		local chanta = GetPlayerPed(play) 

		if IsControlJustPressed(1, Keys['G']) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and not IsEntityDead(chanta) and not IsPedInAnyVehicle(ped, true) then
		...
		elseif IsControlJustPressed(1, Keys['G']) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and IsEntityDead(chanta) and not IsPedInAnyVehicle(ped, true) then

                ESX.ShowNotification("El jugador esta muerto") --- en la linea de arriba se hace un chequeo de si el jugador mas cercano esta muerto o no, si lo esta envia esta notificacion.
		end
	end
end)
